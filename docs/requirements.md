...
| FR-PY-06 | RevenueCat を介した購入状態の同期 | COULD | クラウド課金同期を導入する場合のみ対象。Phase 1 local-first MVP の必須要件ではない |
...
| LF-CACHE-01 | 同一入力ではローカルキャッシュ済み鑑定を返せる | MUST | personalization 要素込みの `inputHash` 検索 |
